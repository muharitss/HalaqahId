...
export function useLaporanPdf() {
  ...
  const getPdfDocument = useCallback(({
    ...
    let docTitle: string;
    if (selectedSantri && selectedSantri !== "") {
      docTitle = `Laporan ${selectedSantri}`;
    } else if (activeHalaqah && activeHalaqah !== "all" && activeHalaqah !== "") {
      docTitle = `Laporan Halaqah ${activeHalaqah}`;
    } else {
    ...
    return { doc, filename, title: docTitle };
  }, [sekolah]);

  const generatePdf = useCallback(async (options: UseLaporanPdfOptions) => {
