...
export default function KelolaAuditLogPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [search, actionFilter, schoolFilter]);

  useEffect(() => {
    fetchLogs();
  }, [page, search, actionFilter, schoolFilter]);
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.message || "Gagal memuat data log audit");
      }
    } catch (error: unknown) {
      toast.error(error instanceof Error ? error.message : "Terjadi kesalahan saat memuat log audit");
    } finally {
      setIsLoading(false);
    }
  };
